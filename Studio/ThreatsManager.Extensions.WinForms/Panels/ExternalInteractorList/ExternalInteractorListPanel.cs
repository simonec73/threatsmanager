...
{
    public partial class ExternalInteractorListPanel : UserControl, IShowThreatModelPanel<Form>, 
        ...
        IDesktopAlertAwareExtension, IPanelOpenerExtension, IExecutionModeSupport
    {
        ...
        public ExternalInteractorListPanel()
        {
            InitializeComponent();
        ...
        private void ModelChildCreated(IIdentity identity)
        {
            ...
            if (identity is IExternalInteractor externalInteractor && IsSelected(externalInteractor, filter, filterSpecial))
            {
        ...
        {
            if (entity is IExternalInteractor externalInteractor)
            {
                HandleEntityShapeEvent(externalInteractor);
            }
        }
        ...
        private void LoadModel()
        {
            try
            {
                ...
                var items = _model.Entities?.OfType<IExternalInteractor>()
                    .OrderBy(x => x.Name)
                    .ToArray();
                var filter = _filter.Text;
                var filterSpecial = EnumExtensions.GetEnumValue<ExternalInteractorListFilter>((string)_specialFilter.SelectedItem);
