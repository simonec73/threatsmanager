...
namespace ThreatsManager.Engine.ObjectModel.Properties
{
    ...
    [Undoable]
    [AssociatedPropertyClass("ThreatsManager.Engine.ObjectModel.Properties.PropertyUrlList, ThreatsManager.Engine")]
    public class UrlListPropertyType : IUrlListPropertyType
    {
        public UrlListPropertyType()
        {

        }

        public UrlListPropertyType([Required] string name, [NotNull] IPropertySchema schema) : this()
        {
            _id = Guid.NewGuid();
            _schemaId = schema.Id;
            _model = schema.Model;
        ...
        public IPropertyType Clone([NotNull] IPropertyTypesContainer container)
        {
            ...
            {
                using (var scope = UndoRedoManager.OpenScope("Clone Url List Property Type"))
                {
                    result = new UrlListPropertyType
                    {
                        _id = _id,
                        _schemaId = schema.Id,
                        _model = schema.Model,
