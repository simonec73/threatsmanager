        ...
        public double Analyze(IThreatModel model, Func<IQualityAnalyzer, IPropertiesContainer, bool> isFalsePositive, out IEnumerable<object> instances)
        {
            ...
            if (entities?.Any() ?? false)
            {
                ...
                foreach (var entity in entities)
                {
                    ...
                    {
                        if (eiRegex == null)
                            eiRegex = new Regex($"^{entity.GetIdentityTypeName()} [0-9]+$");
                        regex = eiRegex;
                    } else if (entity is IProcess)
                    {
                        if (pRegex == null)
                            pRegex = new Regex($"^{entity.GetIdentityTypeName()} [0-9]+$");
                        regex = pRegex;
                    } if (entity is IDataStore)
                    {
                        if (dsRegex == null)
                            dsRegex = new Regex($"^{entity.GetIdentityTypeName()} [0-9]+$");
                        regex = dsRegex;
                    }                  
            ...
            if (groups?.Any() ?? false)
            {
                ...
                {
                    if (tbRegex == null)
                        tbRegex = new Regex($"^{group.GetIdentityTypeName()} [0-9]+$");
