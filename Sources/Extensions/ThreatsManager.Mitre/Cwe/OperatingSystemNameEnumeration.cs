// ------------------------------------------------------------------------------
//  <auto-generated>
//    Generated by Xsd2Code++. Version 5.2.97.0. www.xsd2code.com
//  </auto-generated>
// ------------------------------------------------------------------------------
#pragma warning disable
namespace ThreatsManager.Mitre.Cwe
{
using System;
using System.Diagnostics;
using System.Xml.Serialization;
using System.Runtime.Serialization;
using System.Collections;
using System.Xml.Schema;
using System.ComponentModel;
using System.Xml;
using System.IO;
using System.Text;
using System.Collections.Generic;

/// <summary>
/// The OperatingSystemNameEnumeration simple type contains a list of values corresponding to different operating systems.
/// </summary>
[System.CodeDom.Compiler.GeneratedCodeAttribute("System.Xml", "4.8.4084.0")]
[Serializable]
[XmlTypeAttribute(Namespace="http://cwe.mitre.org/cwe-6")]
public enum OperatingSystemNameEnumeration
{
    AIX,
    Android,
    [XmlEnumAttribute("BlackBerry OS")]
    BlackBerryOS,
    [XmlEnumAttribute("Chrome OS")]
    ChromeOS,
    Darwin,
    FreeBSD,
    iOS,
    macOS,
    NetBSD,
    OpenBSD,
    [XmlEnumAttribute("Red Hat")]
    RedHat,
    Solaris,
    SUSE,
    tvOS,
    Ubuntu,
    watchOS,
    [XmlEnumAttribute("Windows 9x")]
    Windows9x,
    [XmlEnumAttribute("Windows Embedded")]
    WindowsEmbedded,
    [XmlEnumAttribute("Windows NT")]
    WindowsNT,
}
}
#pragma warning restore
